...
class EmailNotification:
    ...
    def send_email(message:str,subject:str,to:list[str]):
        ...
        with smtplib.SMTP_SSL('smtp.gmail.com', 465 , context=context) as gmail:
            gmail.login(EmailNotification.__service_account_email, EmailNotification.__service_account_password)
            for destination_mail in to:
                email_message = EmailMessage()
                email_message['From']=EmailNotification.__service_account_email
                email_message['Subject']= subject if subject else EmailNotification.DEFAULT_SUBJECT
                email_message.set_content(message)
                email_message['To']= destination_mail
                gmail.sendmail(EmailNotification.__service_account_email, destination_mail, email_message.as_string())
            gmail.quit()
